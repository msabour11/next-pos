function overridePOSCustomerSelector() {
  if (
    ...
    // erpnext.PointOfSale.CustomPayment = class extends (
    //   erpnext.PointOfSale.Payment
    // ) {
    //   // constructor({ events, wrapper, settings }) {
    //   //   super({ events, wrapper, settings }); // Call the parent constructor
    //   //   // this.settings = settings; // Add settings parameter

    //   //   // Initialize custom_full_payment or other settings
    //   //   this.custom_full_payment = this.settings.custom_full_payment || false;
    //   //   this.allowed_customer_groups = settings.customer_groups;

    //   //   // Additional initialization if needed
    //   //   this.bind_events(); // Ensure events are bound after initialization
    //   // }
    ...
    erpnext.PointOfSale.Payment.prototype.bind_events = function () {
      ...
      this.$component.on("click", ".submit-order-btn", async () => {
        ...
        console.log("the group name is ", customer_group);
        const is_cach_group = await this.get_cash_group(customer_group);

        console.log("from function db is", is_cach_group);

        console.log("full profile:", pos_profile);
        if (!pos_profile) return;

        // Ensure the code waits for get_is_full_payed to resolve
        // check if payed amount is less than grande total
        try {
          const is_payed = await this.get_is_full_payed(pos_profile);
          ...
          console.error("Error checking full payment:", error);
    ...
    erpnext.PointOfSale.Payment.prototype.get_is_full_payed = function (
      pos_profile
    ) {
      return new Promise((resolve, reject) => {
        frappe.call({
          ...
          callback: (res) => {
            if (res.message) {
              const profile = res.message.custom_full_payment;
              console.log("profile", profile);
              resolve(profile); // Resolve the promise with the profile data
            } else {
    ...
    erpnext.PointOfSale.Payment.prototype.get_cash_group = function (group) {
      return new Promise((resolve, reject) => {
        frappe.db
          .get_value("Customer Group", group, "custom_pos_cach")
          .then((r) => {
            if (r.message) {
              const is_cach = r.message.custom_pos_cach;
              resolve(is_cach);
            } else {
              reject("No group data returned");
            }
          });
      });
    };
...
//         this.events = events;
//         this.custom_full_payment = settings.custom_full_payment;
...
//       bind_events() {
//         const me = this;
//         const full_payment = this.custom_full_payment;
...
//           console.log("full profile:", pos_profile);
//           if (!pos_profile) return;
//           // let is_payed = this.get_is_full_payed(pos_profile);
//           this.get_is_full_payed(pos_profile).then((is_payed) => {
//             console.log("the is pay value is", is_payed);
//             if (is_payed && paid_amount > 0 && paid_amount < grand_total) {
//               frappe.show_alert({
//                 message: __(
//                   "You must complete the pay all order before submitting the order."
//                 ),
//                 indicator: "orange",
//               });
//               frappe.utils.play_sound("error");
